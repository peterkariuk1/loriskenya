  ...
  useEffect(() => {
    const fetchWomenProducts = async () => {
      try {
        ...
        console.log("Visible Women's Products Count:", Math.min(visibleCount, womenProducts.length));
  ...
  return (
    <section className="paginated-grid-section" id="women">
      ...
      {visibleProducts.length === 0 ? (
        <p className="no-products-message">No women's perfumes available at the moment.</p>
      ) : (
